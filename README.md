...
```kotlin
val replay = InteractionReplay(BuildConfig.FLAGDASH_REPLAY_KEY, release = BuildConfig.VERSION_NAME)
    .observeLifecycle(ProcessLifecycleOwner.get())
if (replay.start()) {
    replay.screen("Checkout")
    replay.interaction("buy_tapped", screen = "Checkout")
    replay.stop()
}
```

Android replay is an explicit interaction timeline, not a screen recording. It never captures pixels, view trees, or input values. Sensitive property keys are redacted; use a key scoped only to `replays:write`.
